    ...
    const obtenerRelacionados = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${BASE_URL_MELI}/api/items?tenant=${TENANT_MELI}&limit=50`
        );
        ...
        const autosDisponibles = (data.items || []).filter((auto: ApiCar) => {
          const hasImages =
            (auto.images && auto.images.length > 0) ||
            (auto.firstImage && auto.firstImage.s3ThumbnailUrl);
          return (
            auto.itemId !== currentCarId &&
            hasImages &&
            auto.status === 'active'
          );
        });
        ...
        const autosRelacionados: RelatedCar[] = (
          autosAleatorios as ApiCar[]
        ).map((auto) => {
          const sortedImages = [...(auto.images || [])].sort(
          ...
          const mappedImages = normalizedImages.length
            ? normalizedImages.map((img, index) => ({
                id: `${auto.itemId}-img-${index}`,
                carId: auto.itemId,
                imageUrl: img.s3ImageUrl || img.s3ThumbnailUrl,
                thumbnailUrl: img.s3ThumbnailUrl || img.s3ImageUrl,
                order: img.order ?? index,
              }))
            : [
                {
                  id: `${auto.itemId}-img-placeholder`,
                  carId: auto.itemId,
          ...
          const priceValue = auto.price ? parseFloat(auto.price) : 0;

          return {
            id: auto.itemId,
            brand: auto.brand,
            model: auto.title || auto.model,
            year: auto.year,
            price: {
              valor:
                !Number.isNaN(priceValue) && priceValue > 0 ? priceValue : 0,
              moneda: auto.currencyId || 'ARS',
            },
            active: auto.status === 'active',
            mileage: auto.kilometers !== null ? auto.kilometers : null,
            Images: mappedImages,
          };
        });
